...
ClassificationResult DetectFilePath(const std::string_view s) {
  ...
  if (s.size() >= 3 && std::isalpha(static_cast<unsigned char>(s[0])) &&
      s[1] == ':' && (s[2] == '\\' || s[2] == '/'))
    return {LanguageEnum::FilePath, 0.85f};
  ...
  if (s[0] == '/' && s[1] != '*' && s[1] != '/') {
    ...
    for (const char c: s)
      if (c == '/')
        slashes++;
    if (slashes >= 2)
      return {LanguageEnum::FilePath, 0.80f};
    return {LanguageEnum::FilePath, 0.55f};
  }

  if (s.starts_with("./") || s.starts_with("../"))
    return {LanguageEnum::FilePath, 0.75f};

  if (LooksLikeRelativeSourcePath(s))
    return {LanguageEnum::FilePath, 0.82f};
