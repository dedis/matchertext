...
including a machine-checked proof that delimiting valid matchertext
...
including how matcher-delimiting untrusted input
yields a structural defense against injection attacks
(\cref{sec:host:inject}).

This work remains in an exploration and experimentation phase,
but now reports an initial empirical evaluation of matchertext
on a large corpus of real-world code in \cref{sec:eval}.
Matchertext raises several key questions that this evaluation,
with the preceding design, takes up:
...
For that last question we offer a structural answer in \cref{sec:host:inject},
where matcher-delimiting untrusted input resists injection attacks,
though its empirical effect on defect rates remains open.
