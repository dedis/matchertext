...
Applying matchertext to a particular language is future work of a standard shape, in two parts.
The first is an implementation of the hosting and embedding extensions of \cref{sec:host} in a
mature processor for that language, a compiler, interpreter, or library, selectively enabled
through configuration parameters so that previously valid code is unaffected.
The second is a configurable mode in the same processor that checks and enforces the matchertext
discipline, verifying that matchers match in the source files or strings it handles.
Carried out across enough languages, and converging on a shared configuration, these steps are what
would turn matchertext from a per-language extension into the standardized, cross-language discipline
this paper proposes.
