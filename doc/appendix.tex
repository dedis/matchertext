...
theorem disjoint (h : ∀ x y z, (x, y) ∈ Pi → (z, x) ∈ Pi → False)
    (x : α) : ¬ (Opener Pi x ∧ Closer Pi x) := by ...  -- proof elided; see doc/proof.lean
...
theorem embedding_closed_under_context
    ...
    MT Pi (s_o ++ m ++ s_c) := by ...  -- proof elided; see doc/proof.lean
\end{lstlisting}
