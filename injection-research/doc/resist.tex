...
Then \textsc{FromMatchertext} scans the balance-delimited region once, mapping
each \textsc{esc}$(c)$ back to its matcher $c$ and copying every other character,
so that $\textsc{FromMatchertext}(\textsc{ToMatchertext}(v)) = v$ for every $v$:
\begin{verbatim}
FromMatchertext(m):
  out = []; i = 0
  while i < |m|:
