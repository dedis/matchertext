...
the count carrying an extractable proof-of-concept payload.}
\label{tab:syntax}
\end{table}
...
above any single source.
This linkage is what makes the prevention analysis of \cref{sec:prevent}
possible, since it supplies the real attack strings.
...
For the \textbf{5{,}763} injection CVEs with an extractable PoC payload, we
...
Grouping by identical skeleton yields \textbf{2{,}217} distinct groups
(1{,}878 of them singletons), so specific values never split a group but a
different breakout structure always does.
The largest groups are recognizable attack families: the XSS skeleton
\verb|<TAG> ALERT ( DOCUMENT . COOKIE ) </TAG>| (231 CVEs), the SQL tautology
\verb|<q> OR <q> <n> = <n>| (46), and the code-eval fragment \verb|< ? <id>|
(109).
